...
internal sealed class UpdateWeightDataHandler(IWeightService service) : ICommandHandler<UpdateWeightData, Result>
{
    public async Task<Result> ExecuteAsync(UpdateWeightData command, CancellationToken ct)
    {
        ...
        var response = await service.UpdateAsync(data, ct);
